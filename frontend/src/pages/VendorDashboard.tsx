    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto">
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="border border-ink/15 bg-paper p-6 sm:p-7"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#2E8B72]">Store link</p><Link2 className="mt-8 h-7 w-7 text-[#C94F36]" /><p className="mt-5 font-display text-2xl font-semibold">Share one clear address.</p><p className="mt-3 break-all text-sm leading-relaxed text-ink/65">bridge.com/store/{vendor.slug}</p><button onClick={handleCopyLink} className="mt-7 inline-flex min-h-11 items-center gap-2 bg-[#d6ff57] px-4 text-sm font-semibold text-[#11110f] hover:bg-[#ecffad]"><Copy className="h-4 w-4" />{copied ? "Link copied" : "Copy store link"}</button></div>
        </section>
      </div>
    </DashboardLayout>
  );
