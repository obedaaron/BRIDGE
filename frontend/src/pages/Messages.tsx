...
export function Messages() {
  ...
  return <DashboardLayout><div className="max-w-5xl mx-auto py-2 sm:py-4">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-8"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#2E8B72] mb-3">Inbox</p><h1 className="font-display text-4xl sm:text-5xl font-semibold tracking-tight">Messages.</h1><p className="text-ink/45 mt-3">Keep every conversation and protected deal in one place.</p></div>{unreadTotal > 0 && <div className="rounded-none bg-[#dce9df]/10 px-4 py-3 text-sm"><strong>{unreadTotal}</strong> unread message{unreadTotal === 1 ? "" : "s"}</div>}</div>
      <div className="relative mb-5"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-ink/30" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations" className="w-full rounded-xl border border-ink/10 bg-white py-3 pl-11 pr-4 text-sm outline-none focus:border-[#2E8B72]/60" /></div>
      {conversations === undefined ? <div className="py-20 text-center text-sm text-ink/40">Loading conversations…</div> : visible.length === 0 ? <div className="text-center py-20 bg-paper border border-ink/15 rounded-none"><Inbox className="w-8 h-8 text-ink/20 mx-auto mb-4" strokeWidth={1.5} /><p className="font-medium">{query ? "No conversations found" : "No conversations yet"}</p><p className="text-sm text-ink/40 mt-2">{query ? "Try a different search." : "Start by messaging a store from the marketplace."}</p>{!query && <Link to="/explore" className="inline-block mt-5 text-sm font-medium text-[#2E8B72]">Explore stores</Link>}</div> : <section className="overflow-hidden bg-paper border border-ink/15 rounded-none border border-ink/10 divide-y divide-ink/10">{visible.map((conversation) => <Link key={conversation.id} to={`/messages/${conversation.id}`} className={`group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 hover:bg-ink/[0.025] transition ${conversation.unread_count > 0 ? "bg-[#dce9df]/[0.035]" : ""}`}><div className="relative shrink-0">{conversation.counterpart_type === "vendor" && conversation.vendor_logo ? <img src={conversation.vendor_logo} alt="" className="w-12 h-12 rounded-none object-cover" /> : <div className="w-12 h-12 rounded-none bg-ink/5 flex items-center justify-center font-display font-semibold text-ink/45">{conversation.counterpart_name.charAt(0)}</div>}{conversation.unread_count > 0 && <span className="absolute -right-1 -top-1 min-w-5 h-5 px-1 rounded-full bg-signal text-ink text-[10px] font-bold flex items-center justify-center">{conversation.unread_count > 99 ? "99+" : conversation.unread_count}</span>}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className={`truncate ${conversation.unread_count > 0 ? "font-semibold" : "font-medium"}`}>{conversation.counterpart_name}</p><time className={`shrink-0 text-xs ${conversation.unread_count > 0 ? "text-[#2E8B72] font-medium" : "text-ink/35"}`}>{formatTime(conversation.last_message_at)}</time></div><div className="flex items-center gap-2 mt-1"><span className="text-[10px] uppercase tracking-wider text-ink/35">{conversation.counterpart_type}</span><p className={`truncate text-sm ${conversation.unread_count > 0 ? "text-ink/70 font-medium" : "text-ink/40"}`}>{conversation.last_message || "No messages yet"}</p></div></div><ChevronRight className="w-5 h-5 text-ink/20 group-hover:text-[#2E8B72] shrink-0" /></Link>)}</section>}
    </div></DashboardLayout>;
}
