...
const fallbackPlans: Plan[] = [
  { tier: "free", amountKobo: 0, currency: "NGN", label: "Free", listingLimit: 10, promotionLimit: 0, customization: "Basic logo and store details" },
...
export function Plans() {
  ...
  return <DashboardLayout><div className="max-w-5xl mx-auto"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#2E8B72] mb-3">Store plan</p><h1 className="font-display text-4xl sm:text-5xl font-semibold tracking-tight">Grow on BRIDGE.</h1><p className="mt-3 text-ink/45 max-w-2xl">All prices are in naira and billed monthly through Paystack. Paid plans renew automatically using Paystack’s supported recurring-payment methods.</p>{error && <p className="mt-5 text-sm text-[#2E8B72]">{error}</p>}<div className="grid md:grid-cols-3 gap-5 mt-9">{plans.map((plan) => <div key={plan.tier} className={`bg-paper border border-ink/15 rounded-none border p-6 ${tier === plan.tier ? "border-signal ring-1 ring-signal/20" : "border-ink/10"}`}><p className="font-display text-2xl font-semibold">{plan.label}</p><p className="font-mono text-2xl mt-4">{planPrice(plan)}</p><p className="text-xs text-ink/40 mt-1">per month</p><ul className="mt-6 space-y-3 text-sm text-ink/60"><li className="flex gap-2"><Check className="w-4 h-4 text-[#2E8B72]" />{plan.listingLimit === null ? "Unlimited product listings" : `Up to ${plan.listingLimit} listings`}</li><li className="flex gap-2"><Check className="w-4 h-4 text-[#2E8B72]" />{plan.promotionLimit === null ? "Unlimited active marketplace boosts" : plan.promotionLimit ? `${plan.promotionLimit} active marketplace boosts` : "No marketplace boosts"}</li><li className="flex gap-2"><Check className="w-4 h-4 text-[#2E8B72]" />{plan.customization}</li><li className="flex gap-2"><Check className="w-4 h-4 text-[#2E8B72]" />Protected BRIDGE payments</li></ul>{plan.tier === "free" ? <p className="mt-7 text-sm text-ink/40">{tier === "free" ? "Current plan" : "Available after cancelling paid plan"}</p> : <button onClick={() => choose(plan.tier as "standard" | "premium")} disabled={loading !== null || tier === plan.tier} className="mt-7 w-full bg-[#2E8B72] text-paper rounded-xl py-3 text-sm disabled:opacity-50">{loading === plan.tier ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : tier === plan.tier ? "Current plan" : `Choose ${plan.label}`}</button>}</div>)}</div></div></DashboardLayout>;
}
