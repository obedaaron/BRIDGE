...
export function StarRating({ value, onChange, size = "md", tone = "light" }: StarRatingProps) {
  ...
  return (
    <div className="inline-flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Star
            className={`${cls} ${n <= Math.round(value) ? "text-gold" : tone === "dark" ? "text-white/25" : "text-ink/15"}`}
            strokeWidth={1.5}
